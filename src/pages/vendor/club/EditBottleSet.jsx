...
const EditBottleServiceManager = () => {
   ...
   const [addOns, setAddOns] = useState({
      sparklerShow: initData.addOns.includes('Sparkler Show') || true,
      iceBuckets: initData.addOns.includes('Ice Buckets') || true,
      mixerChaser: initData.addOns.includes('Mixer/Chaser') || true,
      cupsGlassware: initData.addOns.includes('Cups / Glassware') || true,
      showgirls: initData.addOns.includes('Showgirls / Dancers') || true,
      fireShooter: initData.addOns.includes('Fire Shooter') || true,
      djShoutout: initData.addOns.includes('DJ Shout-out') || true,
      vipEntryPass: initData.addOns.includes('VIP Entry Pass') || true
   });
   ...
   const handleSubmit = async () => {
      try {
         ...
         const response = await clubService.updateBottleSet(id, bottleSetPayload);
         console.log('Bottle set updated successfully:', response);
         toast.success('Bottle set Updated successfully!');
         navigate('/dashboard/club/drinks')

      } catch (error) {
         console.error('Error updating bottle set:', error);
         toast.error('Failed to update bottle set. Please try again.');
      }
   };
   ...
   const createNewDrink = async () => {
      ...
      try {
         ...
         const response = await clubService.createDrinkType(id, payload);
   ...
   useEffect(() => {
      ...
      const fetchBottleSet = async () => {
            try {
                const data = await clubService.getBottleSetById(id);
                setBottleSetName(data.name);
                const modifiedDrinks = data.items.map(item => {
                   return { ...item.drinkId, quantity: item.quantity || 1 };
                });
                setInitData(data);
                setBottleSetImage(data.image);
                setSelectedDrinks(modifiedDrinks);
            } catch (error) {
                console.error('Error fetching bottle set:', error);
            } finally {
...
export default EditBottleServiceManager;