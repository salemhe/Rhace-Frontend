...
function ReservationHomePage() {
  ...
  const [showGuestPicker, setShowGuestPicker] = useState(false); // Track guest picker visibility
  ...
  const toggleDatePicker = () => {
    ...
    setShowGuestPicker(false); // Close guest picker if open
  ...
  // Handle Guest picker toggle
  const toggleGuestPicker = () => {
    setShowDatePicker(false); // Close date picker if open
    setShowTimePicker(false); // Close time picker if open
    setShowGuestPicker((prev) => !prev); // Toggle guest picker
    setActiveInput((prev) => (prev === "guest" ? null : "guest")); // Toggle active input
  };
        ...
        <div className="flex flex-col items-center justify-center h-full">
          ...
          <div className="bg-[#F9FAFB] px-5 py-3 max-w-5xl w-full rounded-full mx-auto mt-8 border-2 border-[#E5E7EB] shadow-md relative">
            <div className="flex items-center justify-between gap-4">
              ...
              {/* Search Button */}
              <Link to='/search' className="bg-[#0A6C6D] hover:bg-teal-700 text-white px-6 py-3 rounded-full transition-colors duration-200 flex items-center justify-center min-w-[100px]">
