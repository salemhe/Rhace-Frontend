  ...
  return (
    ...
    >
      <div className="relative h-36 sm:h-44 rounded-2xl sm:rounded-3xl overflow-hidden bg-gray-100 shrink-0">
        ...
        {/* <span
          className={`absolute top-2 left-2 ${cfg.bg} text-white text-[10px] font-bold px-2 py-0.5 rounded-full`}
        >
          {cfg.singular}
        </span> */}
        {/* {vendor.isVerified && (
        ...
        {/* {type === "hotel" && vendor.starRating > 0 ? (
          <span className="absolute bottom-2 left-2 bg-black/50 backdrop-blur-sm text-amber-300 text-[10px] font-semibold px-1.5 py-0.5 rounded">
            {"★".repeat(vendor.starRating)}
          </span>
        ) : vendor.priceRange > 0 ? (
          <span className="absolute bottom-2 left-2 bg-black/50 backdrop-blur-sm text-white text-[10px] font-semibold px-1.5 py-0.5 rounded">
            {PRICE_LABELS[vendor.priceRange]} · {PRICE_DESC[vendor.priceRange]}
          </span>
        ) : null} */}
        {type === "club" && vendor.entryFee != null && (
          <span className="absolute bottom-2 right-2 bg-violet-600/80 backdrop-blur-sm text-white text-[10px] font-semibold px-1.5 py-0.5 rounded">
            {vendor.entryFee === 0
              ? "Free Entry"
              : `₦${vendor.entryFee?.toLocaleString()}`}
          </span>
        )}
        ...
        {/* <button
          ...
          className="absolute top-2 right-2 w-5 h-5 flex items-center justify-center rounded-full bg-white/20 backdrop-blur-sm text-white hover:text-red-400 transition-colors"
        >
      ...
      <div className="flex flex-col p-2.5 sm:p-3 flex-1  gap-1.5">
        ...
        {vendor.vendorTypeCategory &&
          vendor.vendorTypeCategory !== "General" && (
            <p className="text-[10px] text-gray-400 capitalize leading-none">
              {vendor.vendorTypeCategory}
            </p>
          )}
        {tags.length > 0 && (
          <div className="flex flex-wrap gap-1">
            {tags.slice(0, 2).map((tag, i) => {
              ...
              return (
                <span
                  key={i}
                  className="text-gray-500 text-[10px] font-medium  py-0.5 rounded capitalize"
                >
                  {tag.replace(/-/g, " ")}
                  {i < tags.length - 1 && ", "}
                </span>
            ...
            {tags.length > 2 && (
              <span className="bg-gray-100 text-gray-400 text-[10px] px-1.5 py-0.5 rounded">
                +{tags.length - 2}
              </span>
            )}
          </div>
        ...
        <div className="flex items-center gap-1 text-gray-500 text-[10px] sm:text-[14px] mt-auto pt-1">
...
const Radio = ({ val, cur, set, children, count }) => (
  ...
  >
    <div className="flex items-center gap-2">
      <div
        className={`w-3.5 h-3.5 rounded-full border-2 flex items-center justify-center transition-all ${cur === val ? "border-[#0A6C6D]" : "border-gray-300 group-hover:border-[#0A6C6D]"}`}
      >
        {cur === val && (
          <div className="w-1.5 h-1.5 rounded-full bg-[#0A6C6D]" />
        )}
      </div>
      <span
        className={`text-xs ${cur === val ? "font-semibold text-gray-900" : "text-gray-600"}`}
      >
        {children}
      </span>
    </div>
...
const MultiPill = ({ val, cur = [], set, children }) => {
  ...
  return (
    <button
      onClick={() => set(active ? cur.filter((v) => v !== val) : [...cur, val])}
...
const Toggle = ({ cur, set, children }) => (
  <label
    onClick={() => set(cur === "true" ? "" : "true")}
    className="flex items-center justify-between cursor-pointer group py-1 px-1 rounded hover:bg-gray-50 transition-colors"
  >
    <span
      className={`text-xs ${cur === "true" ? "font-semibold text-gray-900" : "text-gray-600"}`}
    >
      {children}
    </span>
    <div
      className={`w-8 h-4 rounded-full transition-all relative shrink-0 ${cur === "true" ? "bg-[#0A6C6D]" : "bg-gray-200"}`}
    >
      <div
        className={`absolute top-0.5 w-3 h-3 bg-white rounded-full shadow transition-all ${cur === "true" ? "left-4" : "left-0.5"}`}
      />
    </div>
  </label>
);
...
const RestaurantFilters = ({ f, onChange }) => (
  <>
    ...
    <div>
      <T>Dietary Options</T>
      <div className="flex flex-wrap gap-1.5">
        {R_DIETARY.map(([val, label]) => (
          <MultiPill
            key={val}
            val={val}
            cur={f.dietaryOptions}
            set={(v) => onChange("dietaryOptions", v)}
          >
            {label}
          </MultiPill>
        ))}
    ...
    <div>
      ...
      <div className="space-y-0.5">
        {R_DINING_STYLES.map(([val, label]) => (
          <Radio
            key={val}
            val={val}
            cur={f.diningStyle}
            set={(v) => onChange("diningStyle", v)}
          >
            {label}
          </Radio>
        ))}
    ...
    <div>
      <T>Seating</T>
      <div className="flex flex-wrap gap-1.5">
        {R_SEATING.map(([val, label]) => (
          <MultiPill
            key={val}
            val={val}
            cur={f.seatOptions}
            set={(v) => onChange("seatOptions", v)}
          >
            {label}
          </MultiPill>
        ))}
    ...
    <div>
      <T>Occasion</T>
      <div className="flex flex-wrap gap-1.5">
        {R_OCCASIONS.map(([val, label]) => (
          <MultiPill
            key={val}
            val={val}
            cur={f.occasionTags}
            set={(v) => onChange("occasionTags", v)}
          >
            {label}
          </MultiPill>
        ))}
    ...
    <div>
      <T>Meal Time</T>
      <div className="flex flex-wrap gap-1.5">
        {R_MEAL_TIMES.map(([val, label]) => (
          <MultiPill
            key={val}
            val={val}
            cur={f.mealTimes}
            set={(v) => onChange("mealTimes", v)}
          >
            {label}
          </MultiPill>
        ))}
    ...
    <div>
      ...
      <div className="space-y-0.5">
        {R_RESERVATION.map(([val, label]) => (
          <Radio
            key={val}
            val={val}
            cur={f.reservationPolicy}
            set={(v) => onChange("reservationPolicy", v)}
          >
            {label}
          </Radio>
        ))}
    ...
    <div>
      <T>Extras</T>
      <div className="space-y-0.5">
        <Toggle cur={f.hasParking} set={(v) => onChange("hasParking", v)}>
          Has Parking
        </Toggle>
        <Toggle
          cur={f.hasOutdoorSeating}
          set={(v) => onChange("hasOutdoorSeating", v)}
        >
          Outdoor Seating
        </Toggle>
...
const HotelFilters = ({ f, onChange }) => (
  <>
    <div>
      <T>Star Rating</T>
      <div className="space-y-0.5">
        {H_STAR_RATINGS.map(([val, label]) => (
          <Radio
            key={val}
            val={val}
            cur={f.starRating}
            set={(v) => onChange("starRating", v)}
          >
            {label}
          </Radio>
        ))}
    ...
    <div>
      <T>Property Type</T>
      <div className="space-y-0.5">
        {H_PROPERTY_TYPE.map(([val, label]) => (
          <Radio
            key={val}
            val={val}
            cur={f.propertyType}
            set={(v) => onChange("propertyType", v)}
          >
            {label}
          </Radio>
        ))}
    ...
    <div>
      <T>Amenities</T>
      <div className="flex flex-wrap gap-1.5">
        {H_AMENITIES.map(([val, label]) => (
          <MultiPill
            key={val}
            val={val}
            cur={f.amenities}
            set={(v) => onChange("amenities", v)}
          >
            {label}
          </MultiPill>
        ))}
    ...
    <div>
      <T>Meal Plan</T>
      <div className="space-y-0.5">
        {H_MEAL_PLANS.map(([val, label]) => (
          <Radio
            key={val}
            val={val}
            cur={f.mealPlan}
            set={(v) => onChange("mealPlan", v)}
          >
            {label}
          </Radio>
        ))}
    ...
    <div>
      <T>Cancellation</T>
      <div className="space-y-0.5">
        {H_CANCEL.map(([val, label]) => (
          <Radio
            key={val}
            val={val}
            cur={f.cancellationPolicy}
            set={(v) => onChange("cancellationPolicy", v)}
          >
            {label}
          </Radio>
        ))}
    ...
    <div>
      <T>Accessibility</T>
      <div className="flex flex-wrap gap-1.5">
        {H_ACCESSIBILITY.map(([val, label]) => (
          <MultiPill
            key={val}
            val={val}
            cur={f.accessibilityFeatures}
            set={(v) => onChange("accessibilityFeatures", v)}
          >
            {label}
          </MultiPill>
        ))}
    ...
    <div>
      <T>Extras</T>
      <div className="space-y-0.5">
        <Toggle cur={f.instantBook} set={(v) => onChange("instantBook", v)}>
          Instant Book
        </Toggle>
        <Toggle cur={f.petFriendly} set={(v) => onChange("petFriendly", v)}>
          Pet Friendly
        </Toggle>
        <Toggle cur={f.openNow} set={(v) => onChange("openNow", v)}>
          Open Now
...
const ClubFilters = ({ f, onChange }) => (
  <>
    <div>
      <T>Venue Type</T>
      <div className="space-y-0.5">
        {C_VENUE_TYPES.map(([val, label]) => (
          <Radio
            key={val}
            val={val}
            cur={f.venueType}
            set={(v) => onChange("venueType", v)}
          >
            {label}
          </Radio>
        ))}
    ...
    <div>
      <T>Music Genre</T>
      <div className="flex flex-wrap gap-1.5">
        {C_GENRES.map(([val, label]) => (
          <MultiPill
            key={val}
            val={val}
            cur={f.musicGenres}
            set={(v) => onChange("musicGenres", v)}
          >
            {label}
          </MultiPill>
        ))}
    ...
    <Hr />
    <div>
      <T>Live Performances</T>
      <div className="flex flex-wrap gap-1.5">
        {C_PERFORMANCES.map(([val, label]) => (
          <MultiPill
            key={val}
            val={val}
            cur={f.livePerformanceTypes}
            set={(v) => onChange("livePerformanceTypes", v)}
          >
            {label}
          </MultiPill>
        ))}
    ...
    <div>
      <T>Dress Code</T>
      <div className="space-y-0.5">
        {C_DRESS_CODES.map(([val, label]) => (
          <Radio
            key={val}
            val={val}
            cur={f.dressCode}
            set={(v) => onChange("dressCode", v)}
          >
            {label}
          </Radio>
        ))}
    ...
    <div>
      <T>Age Policy</T>
      <div className="space-y-0.5">
        {C_AGE_POLICY.map(([val, label]) => (
          <Radio
            key={val}
            val={val}
            cur={f.agePolicy}
            set={(v) => onChange("agePolicy", v)}
          >
            {label}
          </Radio>
        ))}
...
const FilterPanel = ({
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {hasFilters && (
            <button
              onClick={onClear}
              className="text-[11px] font-bold text-[#0A6C6D] hover:underline flex items-center gap-0.5"
            >
          ...
          {onClose && (
            <button
              onClick={onClose}
              className="p-1 rounded border border-gray-200 text-gray-400 hover:text-gray-700 transition-colors"
            >
              <X className="w-3 h-3" />
            </button>
          )}
      ...
      {!f.type && (
        <>
          <div>
            <T>Type</T>
            <div className="space-y-0.5">
              <Radio val="" cur={f.type} set={(v) => onChange("type", v)}>
                All Types
              </Radio>
              <Radio
                val="restaurant"
                cur={f.type}
                set={(v) => onChange("type", v)}
                count={typeCount("restaurant")}
              >
                Restaurants
              </Radio>
              <Radio
      ...
      {f.type === "restaurant" && (
        <RestaurantFilters f={f} onChange={onChange} />
      )}
...
const SearchPage = () => {
  ...
  return (
    <div className="min-h-screen mt- bg-gray-50">
      {/* <UserHeader /> */}

      <div className="z-40 relative bg-white/95 ">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-2.5 sm:py-3">
          <div className="flex z-10 gap-2 flex-wrap justify-between">
            ...
            <div className="flex flex-1 mt-4 sm:mt-0 sm:flex-1 sm:w-auto sm:max-w-2xl sm:mx-auto gap-1.5 sm:gap-3 order-last sm:order-0">
              <div className="flex-1 relative ">
                <div
                  className={`flex items-center gap-2 bg-gray-50 border rounded-full px-3 py-2.5 sm:py-3.5 transition-all ${
                    showDropdown
                      ? "border-[#0A6C6D] ring-2 ring-[#0A6C6D]/10 rounded-b-none"
                ...
                >
                  ...
                  <input
                    ...
                    placeholder={
                      filters.type
                        ? `Search ${TYPE_CONFIG[filters.type]?.label.toLowerCase()}...`
                          ...
                          trending.slice(0, 6).map((v) => {
                            ...
                            return (
                              <div
                                ...
                                className="flex items-center gap-2.5 px-3 sm:px-4 py-2 hover:bg-gray-50 cursor-pointer"
                              >
            ...
            {/* <button
              ...
              className="lg:hidden relative px-2.5 py-2.5 border border-gray-200 rounded-xl text-gray-600 hover:border-gray-400 transition-colors shrink-0"
            >
              <SlidersHorizontal className="w-4 h-4" />
              {activeFilterCount > 0 && (
                <span className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-[#0A6C6D] text-white text-[9px] font-black rounded-full flex items-center justify-center">
                  {activeFilterCount}
                </span>
              )}
            </button> */}
            ...
            {profile ? (
              <div className="flex items-center space-x-4">
                <button
                  className={`hidden md:flex p-2 rounded-full transition-colors duration-200 ${"text-gray-700 hover:bg-gray-100"}`}
                >
                ...
                <div className="relative text-gray-700" ref={dropdownRef}>
                  <button
                    ...
                    className={`flex items-center space-x-3 px-2 py-2 rounded-full transition-colors duration-200 ${"outline outline-gray-200 hover:bg-gray-50"}`}
                  >
                    {loading ? (
                    ...
                    {isMenuOpen ? (
                      <ChevronUp
                        className={`w-5 h-5 ${scrolled ? "text-gray-700" : "text-white"}`}
                      />
                    ) : (
                      <ChevronDown
                        className={`w-5 h-5 ${scrolled ? "text-gray-700" : "text-white"}`}
          ...
          <div className="flex flex-col  items-cente relative  h-14 mt-8.5  hide-scrollbar overflow-x-auto scrollbar-hide ">
            <div className="flex h-full ">
              {[
                { val: "", label: "All", icon: <Search className="w-3 h-3" /> },
                {
                  ...
                  icon: (
                    <figure className="w-3 h-3 flex items-center shrink-0">
                      <SvgIcon isActive={filters.type !== "restaurant"} />
                    </figure>
                  ),
                },
                {
                  ...
                  icon: (
                    <figure className="w-3 h-3 flex items-center shrink-0">
                      <SvgIcon2 className="text-amber-200" />
                    </figure>
                  ),
                },
                {
                  ...
                  icon: (
                    <figure className="w-3 h-3 flex items-center shrink-0">
                      <SvgIcon3 isActive={filters.type !== "club"} />
                    </figure>
                  ),
                },
              ].map(({ val, label }) => (
                <buttton
                  ...
                  key={val}
                  className={` items-center flex  relative cursor-pointer h-full hover:bg-gray-100 transition-colors duration-300 transform `}
                >
                  <div
                    className={`flex items-center px-[18px] md:px-12 lg:px-26 gap-1.5 w-full h-full whitespace-nowrap transition-all  ${
                      filters.type === val
                        ? " text-[#0A6C6D] font-bold "
                        : " text-[#0A6C6D]   hover:text-[#0A6C6D] font-medium"
                    }`}
                  >
                    {" "}
                    {label}
                  ...
                  <div
                    className={`h-1 w-full absolute bottom-0 z-30 rounded bg-[#0A6C6D] ${filters.type === val ? "opacity-100" : "opacity-0"}`}
                  ></div>
                </buttton>
              ))}
            </div>
            <div className="w-full absolute rounded bottom-0 bg-gray-100 h-1"></div>
          </div>
        </div>
      </div>
            ...
            <div className="flex-1 min-w-0">
              {!isLoading && (
                <div className="flex items-center justify-between mb-3 gap-2 flex-wrap">
                  ...
                  {/* <select
                    ...
                    className="hidden sm:block text-xs border border-gray-200 rounded-lg px-2.5 py-1.5 bg-white text-gray-600 focus:outline-none focus:border-[#0A6C6D] cursor-pointer shrink-0"
                  >
              ...
              {error && (
                <div className="flex flex-col items-center gap-3 py-16 text-center">
                  ...
                  <button
                    onClick={() => submitSearch(activeQuery)}
        ...
        <div className="max-w-4xl mx-auto px-3 sm:px-6 py-8 sm:py-14">
          <div className="text-center mb-6 sm:mb-8">
            ...
            <p className="text-gray-400 text-sm">
              Find and book the best spots near you
            </p>
          </div>
                ...
                >
                  ...
                  <div className="absolute inset-0 flex flex-col justify-between p-3 sm:p-3.5">
                    <div className="w-7 h-7 bg-white/20 backdrop-blur-sm rounded-lg flex items-center justify-center">
                      <figure className="w-3.5 h-3.5 flex items-center">
                        {icon}
                      </figure>
                    </div>
                    <div>
