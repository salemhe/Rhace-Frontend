              ...
              <div className="rounded-2xl bg-white border">
                <div className=" divide-y">
                  <div className="flex p-4 text-black justify-between">
                    ...
                    <button onClick={() => setPage(0)} className="flex gap-2 font-medium text-[#0A6C6D]"><Plus /> Add more</button>
                  </div>
                  <div className="space-y-4 p-4">
                    {/* Selected Combos */}
                    {comboItems.filter((item) => item.selected).map((item) => (
                      <div
                        key={`combo-${item._id}`}
                      ...
                      >
                        <div className="flex justify-between items-center">
                          ...
                          <button onClick={() => handleRemoveCombo(item._id)}>
                    ...
                    {/* Selected Tables with Quantities */}
                    {table.filter((item) => item.selected).map((item) => (
                      <div
                        key={`table-${item._id}`}
                        className="space-y-4 px-2 py-3 rounded-2xl border border-[#E5E7EB] bg-[#F9FAFB]"
                      >
                        <div className="flex justify-between items-center">
                          ...
                          <button onClick={() => handleRemoveTable(item._id)}>
                            <X className="text-[#606368] shrink-0" />
                          </button>
                        </div>
                        <div className="flex justify-between items-center">
                          <p className="text-xs text-[#111827]">
                            Quantity: {item.quantity || 1} {item.quantity > 1 ? 'tables' : 'table'}
                          </p>
                          <p className="text-sm text-[#111827]">
                    ...
                    {/* Selected Bottles */}
                    {bottleItems.filter((item) => item.quantity > 0).map((item) => (
                      <div
                        key={`bottle-${item._id}`}
                        className="space-y-4 px-2 py-3 rounded-2xl border border-[#E5E7EB] bg-[#F9FAFB]"
                      >
                        <div className="flex justify-between items-center">
                          <p className="text-sm text-[#111827]">{item.name}</p>
                          <button onClick={() => handleRemoveBottle(item._id)}>
                            <X className="text-[#606368] shrink-0" />
                          </button>
                        </div>
                        <div className="flex justify-between items-center">
                    ...
                    {/* VIP Extras */}
                    {vipExtraItems.length > 0 && vipExtraItems.map((item, index) => (
                      <div
                        key={`vip-${index}`}
                        className="space-y-4 px-2 py-3 rounded-2xl border border-[#E5E7EB] bg-[#F9FAFB]"
                      >
                      ...
                      <div className="divide-y">
                        ...
                        >
                          <div className="space-y-1">
                            ...
                            <p className="text-xs">
                              Pay ₦{Math.round(calculatedTotals.grandTotal / 2).toLocaleString()} now, and ₦{Math.round(calculatedTotals.grandTotal / 2).toLocaleString()} on {date ? format(date, "do MMM, yyyy") : "the day of your arrival"}. No extra fees
                            </p>
                          </div>
                          <svg width="20" height="20" viewBox="0 0 20 20" className="shrink-0" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <rect x="0.5" y="0.5" width="19" height="19" rx="9.5" stroke="#0A6C6D" />
