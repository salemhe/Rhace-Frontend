...
export const SearchBar = ({
  ...
  inputRef, submitSearch, filters,
  showDropdown, showRecent, showTrending, showSuggestions,
  recentSearches, setRecentSearches,
  trending, isTrendLoading,
  suggestions, isSugLoading,
}) => {
  ...
  return (
    <div className="flex-1 relative">
      {/* Input wrapper */}
      <div className={`flex items-center gap-2 bg-gray-50 border rounded-full px-3 py-2.5 sm:py-3.5 transition-all ${
        showDropdown
          ...
          {showRecent && (
            <div>
              <div className="flex items-center justify-between px-3 sm:px-4 pt-2.5 pb-1">
                ...
                <button onMouseDown={e => e.preventDefault()} onClick={handleClearRecent}
                  className="text-[10px] font-bold text-[#0A6C6D]">Clear all</button>
              </div>
              {recentSearches.map(term => (
                <div key={term} onMouseDown={e => e.preventDefault()} onClick={() => submitSearch(term)}
                  ...
                  <button
                    ...
                    onClick={e => { e.stopPropagation(); delRecent(term); setRecentSearches(getRecent()); }}
            ...
            <div>
              ...
              {isTrendLoading ? (
                ...
                trending.slice(0, 6).map(v => (
                  <VendorDropdownRow key={v._id} vendor={v} onClick={() => submitSearch(v.businessName)} />
                ))
              )}
            </div>
          )}
          ...
          {showSuggestions && (
            <div>
              ...
              {isSugLoading ? (
                ...
                suggestions.map(v => (
                  <VendorDropdownRow key={v._id} vendor={v} onClick={() => submitSearch(v.businessName)} showArrow />
                ))
              ) : (
                <p className="px-3 py-4 text-center text-sm text-gray-400">No results for "{inputValue}"</p>
              )}
              {inputValue.trim() && (
                <div onMouseDown={e => e.preventDefault()} onClick={() => submitSearch(inputValue)}
    ...
    <div onMouseDown={e => e.preventDefault()} onClick={onClick}
      ...
      <div className="w-8 h-8 rounded-lg overflow-hidden bg-gray-100 shrink-0">
        {vendor.profileImages?.[0] ? (
          <img src={vendor.profileImages[0]} alt="" className="w-full h-full object-cover" />
        ) : (
          <div className={`w-full h-full flex items-center justify-center text-xs font-black ${cfg?.light || "text-gray-400"}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[11px] text-gray-400 capitalize">
          {cfg?.singular}
          {vendor.vendorTypeCategory && vendor.vendorTypeCategory !== "General" ? ` · ${vendor.vendorTypeCategory}` : ""}
      ...
      {showArrow && <ArrowUpRight className="w-3 h-3 text-gray-200 group-hover:text-gray-400 shrink-0" />}
    </div>
  );
};