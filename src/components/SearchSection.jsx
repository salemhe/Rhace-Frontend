...
const TYPE_CONFIG = {
  restaurant: { label: "Restaurants", singular: "Restaurant", path: "restaurants" },
...
const saveRecent = (t) => {
  ...
  localStorage.setItem(
    LS_RECENT,
    JSON.stringify(
      [t.trim(), ...getRecent().filter((s) => s.toLowerCase() !== t.toLowerCase())].slice(0, MAX_RECENT),
    ),
  );
};
const delRecent = (t) =>
...
const normalizeSuggestion = (item, fallbackType) => {
  ...
  if (typeof item === "string")
    return { _id: item, businessName: item, vendorType: fallbackType || "restaurant", profileImages: [], address: "", rating: 0 };
  return {
...
const searchSvc = {
  suggestions: async (q, type, location) => {
    ...
    try {
      ...
      const normalized = raw.map((i) => normalizeSuggestion(i, type)).filter(Boolean);
...
const SearchDropdown = ({
  ...
  useEffect(() => {
    setIsTrendLoad(true);
    searchSvc.trending(activeType || undefined)
      .then(setTrending).catch(() => {}).finally(() => setIsTrendLoad(false));
  }, [activeType]);
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      searchSvc.suggestions(inputValue, activeType || undefined, userLocation)
    ...
    <div className="absolute left-0 right-0 top-full w-full md:w-88 bg-white rounded-2xl shadow-2xl z-[999] max-h-80 overflow-y-auto">
      ...
      {showRecent && (
        <div>
          <div className="flex items-center justify-between px-4 pt-2.5 pb-1">
            ...
            <button onMouseDown={(e) => e.preventDefault()} onClick={() => { localStorage.removeItem(LS_RECENT); setRecentSearches([]); }}
              className="text-[10px] font-bold text-[#0A6C6D]">Clear all</button>
          </div>
          {recentSearches.map((term) => (
            <div key={term} onMouseDown={(e) => e.preventDefault()} onClick={() => onSelect(term)}
              ...
              <button onMouseDown={(e) => e.stopPropagation()} onClick={(e) => { e.stopPropagation(); onDeleteRecent(term); }}
        ...
        <div>
          ...
          {isTrendLoad ? (
            ...
            trending.slice(0, 6).map((v) => {
              ...
              return (
                <div key={v._id} onMouseDown={(e) => e.preventDefault()} onClick={() => onSelect(v.businessName)}
                  className="flex items-cente gap-2.5 px-4 py-2 hover:bg-gray-50 cursor-pointer">
                  <div className="w-8 h-8 rounded-lg overflow-hidden bg-gray-100 shrink-0">
                    {v.profileImages?.[0]
                      ? <img src={v.profileImages[0]} alt="" className="w-full h-full object-cover" />
                      : <div className="w-full h-full flex items-center justify-center text-xs font-black text-gray-300">{v.businessName?.[0]}</div>}
                  </div>
      ...
      {showSuggestions && (
        <div>
                ...
                <div key={v._id} onMouseDown={(e) => e.preventDefault()} onClick={() => onSelect(v.businessName)}
                  ...
                  <div className="flex-1  justify-items-start min-w-0">
                    ...
                    <p className="text-[11px] text-gray-400 capitalize">
                      {cfg?.singular}
                      {v.vendorTypeCategory && v.vendorTypeCategory !== "General" ? ` · ${v.vendorTypeCategory}` : ""}
          ...
          {inputValue.trim() && (
            <div onMouseDown={(e) => e.preventDefault()} onClick={() => onSubmit(inputValue)}
...
const TAB_TO_TYPE = { restaurants: "restaurant", hotels: "hotel", clubs: "club" };
...
const SearchSection = ({ activeTab, onSearch }) => {
  ...
  const timeSlots = {
    restaurants: ['09:00 AM','09:30 AM','10:00 AM','10:30 AM','11:00 AM','11:30 AM',
      '12:00 PM','12:30 PM','01:00 PM','01:30 PM','02:00 PM','02:30 PM',
      '03:00 PM','03:30 PM','04:00 PM','04:30 PM','05:00 PM','05:30 PM',
      '06:00 PM','06:30 PM','07:00 PM','07:30 PM','08:00 PM','08:30 PM'],
    hotels: ['09:00 AM','09:30 AM','10:00 AM','11:30 AM','01:00 PM','02:00 PM',
      '04:00 PM','04:30 PM','05:00 PM','06:00 PM','06:30 PM','07:30 PM','08:00 PM','09:00 PM'],
    clubs: ['09:00 PM','09:30 PM','10:00 PM','10:30 PM','11:00 PM','11:30 PM',
      '12:00 AM','12:30 AM','01:00 AM','01:30 AM','02:00 AM','02:30 AM','03:00 AM'],
  };

  const placeholder =
    activeTab === "restaurants" ? "Enter Restaurant or Cuisine"
    : activeTab === "hotels"    ? "Enter Hotel name or area"
    : "Enter Club name or area";

  return (
    <div
      className="bg-white z-50 absolute top-6 sm:top-15 w-full sm:w-full mx-auto left-0 right-0 rounded-2xl lg:rounded-full justify-center mb-8 shadow-[0px_34px_10px_0px_rgba(122,122,122,0.00)] outline-2 outline-gray-200"
    >
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5">

        {/* ── Search input with live dropdown ── */}
        <div className="flex flex-col border-b py-2 sm:py-4 px-4 sm:px-6 sm:border-b-0 sm:border-r border-gray-200 col-span-1 sm:col-span-2 lg:col-span-1 relative" ref={wrapperRef}>
          ...
          <div className={`flex items-center gap-1.5 transition-colors `}>
            ...
            />
            {inputValue && (
              <button onMouseDown={(e) => e.preventDefault()} onClick={() => setInputValue("")}
          ...
          <SearchDropdown
            ...
            onDeleteRecent={(term) => { delRecent(term); setRecentSearches(getRecent()); }}
            onClearRecent={() => { localStorage.removeItem(LS_RECENT); setRecentSearches([]); }}
          />
        </div>
        ...
        <div className="grid grid-cols-2 col-span-1 sm:col-span-2 lg:col-span-2">
          ...
          <div className="flex flex-col justify-center py-2 sm:py-4 px-4 sm:px-6 border-b sm:border-b-0 lg:border-r border-gray-200">
            ...
            <TimeDropdown selectedTime={time} slots={timeSlots[activeTab] || timeSlots.restaurants} onChange={setTime} />
          </div>
        </div>
...
export const SearchSectionTwo = ({ onSearch, searchData, activeTab }) => {
  ...
  useEffect(() => {
    const src = searchData || (() => { try { return JSON.parse(localStorage.getItem("searchData")); } catch { return null; } })();
    if (!src) return;
    if (src.query) setInputValue(src.query);
    if (src.date)  setDate(new Date(src.date));
    if (src.time)  setTime(src.time);
    if (src.guests) setGuests({ adults: parseInt(src.guests, 10) || 2, children: 0, infants: 0 });
  }, [searchData]);

  // close on outside click
  useEffect(() => {
    const handle = (e) => {
      const outsideForm    = formRef.current    && !formRef.current.contains(e.target);
      const outsideWrapper = wrapperRef.current && !wrapperRef.current.contains(e.target);
      if (outsideForm)    { if (isExpanded && isMobile) setIsExpanded(false); }
  ...
  const submit = useCallback((term) => {
    ...
    if (onSearch) {
      ...
      navigate(buildSearchUrl({ q, type: vendorType, date: newData.date, time: newData.time, guests: totalGuests }));
    }
  }, [inputValue, guests, date, time, vendorType, activeTab, isMobile, navigate, onSearch]);

  const placeholder =
    activeTab === "restaurants" ? "Search restaurants or cuisines"
    : activeTab === "hotels"    ? "Search hotels"
    : "Search clubs";

  // ── Desktop ──────────────────────────────────────────────────────────────
  return (
    <form ref={formRef} onSubmit={(e) => { e.preventDefault(); submit(); }} className="mx-auto w-full max-w-3xl">

      {/* Desktop */}
      <div className="hidden sm:block relative" ref={wrapperRef}>
        <div className={`h-16 bg-white rounded-full justify-between shadow-lg flex items-center px-2 sm:px-4 gap-2 sm:gap-0 border-2 transition-colors ${isFocused ? "border-[#0A6C6D]" : "border-transparent"}`}>
          <div className="flex flex-col flex-1 justify-center h-full px-4 border-r border-gray-200 min-w-0 w-1/4 relative">
            ...
            <div className="flex items-center gap-1">
              <input
                ...
                onChange={(e) => setInputValue(e.target.value)}
                onFocus={() => setIsFocused(true)}
                onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); submit(); } if (e.key === "Escape") setIsFocused(false); }}
              ...
              />
              {inputValue && (
                <button type="button" onMouseDown={(e) => e.preventDefault()} onClick={() => setInputValue("")}
                  className="text-gray-400 hover:text-gray-700 shrink-0"><X className="w-3.5 h-3.5" /></button>
              )}
            </div>
            ...
            <SearchDropdown
              ...
              onSelect={submit}
              onSubmit={submit}
              onDeleteRecent={(t) => { delRecent(t); setRecentSearches(getRecent()); }}
              onClearRecent={() => { localStorage.removeItem(LS_RECENT); setRecentSearches([]); }}
            />
          </div>
            ...
            <div className="bg-white rounded-2xl w-full shadow-xl animate-in slide-in-from-bottom-5 duration-200">
              <div className="p-4">
                <div className="flex justify-end mb-2">
                  <button type="button" onClick={() => setIsExpanded(false)} className="p-2 rounded-full hover:bg-gray-100">
                ...
                <div className="flex flex-col mb-4 pb-4 border-b border-gray-200">
                  ...
                  <div className={`flex items-center gap-1 border-b-2 pb-1 transition-colors ${isFocused ? "border-[#0A6C6D]" : "border-gray-200"}`}>
                    <input
                      ref={inputRef}
                      autoFocus
                      type="text"
                    ...
                    {inputValue && (
                      <button type="button" onMouseDown={(e) => e.preventDefault()} onClick={() => setInputValue("")}
                        className="text-gray-400 shrink-0"><X className="w-3.5 h-3.5" /></button>
                    )}
                  </div>
